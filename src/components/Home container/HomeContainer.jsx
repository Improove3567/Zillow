            ...
            <div className='py-2 flex-1 flex items-center relative'>
                ...
                <div className='w-full h-full absolute top-0 left-0 flex items-center justify-center px-32 py-4 gap-4 flex-wrap'>
                    {heroData && heroData.map((el) => (
                        <div key={el.id} className='w-190 p-4 bg-cardOverlay backdrop-blur-md rounded-3xl flex flex-col items-center justify-center'>
                            <img className='w-40 -mt-20' src={el.imageSrc} alt='i1' />
                            <p className='text-xl font-semibold text-textColor mt-4'>{el.name}</p>

                            <p className='text-sm text-lighttextGray font-semibold my-3'>{el.desp}</p>
